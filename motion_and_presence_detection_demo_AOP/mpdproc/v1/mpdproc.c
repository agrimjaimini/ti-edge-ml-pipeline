...
void assignPointsToZones(MPDProcObj *mpdProcObj, motionTrackerLocal *zonesLocal, uint8_t isMajor)
{
    ...
    if (dbscanOutputData.numClusters > 0)
    {
        ...
        for (i = 0; i < mpdProcObj->staticCfg.sceneryParams.numVirtualZones; i++)
        {
            ...
            {
#ifdef CLUSTERING_2D
                int occupancy = (*fun_ptr)(&(dbscanOutputData.clusterCentroids[2 * j]), fun_args);
#else
                int occupancy = (*fun_ptr)(&(dbscanOutputData.clusterCentroids[3 * j]), fun_args);
#endif
                if (occupancy > 0)
                {
#ifdef CLUSTERING_2D
...
#else
                    rngCluster = (float)sqrt(pow((dbscanOutputData.clusterCentroids[3 * j] - mpdProcObj->staticCfg.sceneryParams.sensorPosition.x), 2) + pow((dbscanOutputData.clusterCentroids[3 * j + 1] - mpdProcObj->staticCfg.sceneryParams.sensorPosition.y), 2) + pow((dbscanOutputData.clusterCentroids[3 * j + 2] - mpdProcObj->staticCfg.sceneryParams.sensorPosition.z), 2));
#endif
...
void stateMachine(MPDProcObj *mpdProcObj, motionTrackerLocal *zonesLocal)
{
    ...
    for (i = 0; i < mpdProcObj->staticCfg.sceneryParams.numVirtualZones; i++)
    {
        ...
        {
            case STATE_EMPTY:
                if (mpdProcObj->staticCfg.motionMode == 1)
                {
                ...
                else
                {
                    ...
                    else if (zonesLocal[i].numPointsMinor >= (int)mpdProcObj->staticCfg.minorStateParamCfg.pointThre1 ||
                             (zonesLocal[i].numPointsMinor >= (int)mpdProcObj->staticCfg.minorStateParamCfg.pointThre2 && zonesLocal[i].totalSnrMinor >= mpdProcObj->staticCfg.minorStateParamCfg.snrThre2))
                    {
                        mpdProcObj->res.zones[i].state            = STATE_MINOR;
                        mpdProcObj->res.zones[i].minor2emptyCount = 0;
